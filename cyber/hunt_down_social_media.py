...
def banner():
 ...
 | |__| | | (_| \__ \__ \  \ V /| || |_| | |_| |
  ...
  ''')
def search():
    GREEN(f"[+] sSearching for username {username}: ")
    time.sleep(0.5)
    print('............')
    time.sleep(0.5)
    print('............')
    time.sleep(0.5)
    GREEN('[+] Looking-Glass v1.0 is working')
    time.sleep(0.5)
    print('............')
    time.sleep(0.5)
    print('............')
    ...
    for url in WEBSITES:
        ...
        if r.status_code==200:
            if match:
                GREEN('[+] FOUND MATCHES')
                match=not match
            YELLOW(f'{url} - {r.status_code} -OK')
            if username in r.text:
                GREEN(f'POSITIVE MATCH: Username:{username}  - text has been detected in the url {url}')
            else:
