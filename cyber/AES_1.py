class AES:
    ...
    def _key_expansion(self, key):
        ...
        # Initialize round keys array with correct size
    ...
    def _shift_rows(self, state):
        ...
        # Create a temporary state to avoid overwriting issues
        temp_state = state[:]
    ...
    def _inv_shift_rows(self, state):
        ...
        state[1] = temp_state[13]
        state[5] = temp_state[1]
        state[9] = temp_state[5]
        state[13] = temp_state[9]
        ...
        state[3] = temp_state[7]
        state[7] = temp_state[11]
        state[11] = temp_state[15]
        state[15] = temp_state[3]
    ...
    def _mix_columns(self, state):
        ...
        for i in range(4):
            ...
            self._mix_single_column(column)
            state[i + 0 * 4] = column[0]
            state[i + 1 * 4] = column[1]
            state[i + 2 * 4] = column[2]
            state[i + 3 * 4] = column[3]
    ...
    def _inv_mix_columns(self, state):
        ...
        for i in range(4):
            ...
            u = self._xtime(self._xtime(column[0] ^ column[2]))
            v = self._xtime(self._xtime(column[1] ^ column[3]))
            column[0] ^= u
            column[1] ^= v
            column[2] ^= u
            column[3] ^= v
...
if __name__ == "__main__":
    try:
        ...
        assert text == decrypted_text.decode('utf-8')
        assert binary_data == decrypted_binary
        assert short_data == decrypted_short
        print("\nAll tests passed!")
